...
- **관리** — 이동 · 이름 변경 · 메타 수정 · 폴더 · 공유 링크 · **삭제** (CLI + 뷰어).
...
| `rm <ref> --confirm` | 문서 삭제(하이라이트·공유 링크 함께 제거) |
| `folder mkdir\|rename\|rmdir <path>` | 폴더 관리 |
| `folder rmdir <path> --recursive --confirm` | 폴더와 하위 문서·폴더를 통째로 삭제 |
...
- **로그인 시**: 폴더 생성/이름변경/삭제, 파일 드래그 이동, 파일 이름변경, 메타·공개범위 편집, 공유 링크, **문서·폴더 삭제**(대상 이름을 그대로 입력해야 실행).
